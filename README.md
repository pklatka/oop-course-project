...
Final project for object-oriented programming course at AGH University of Science and Technology.

The topic of the final project was to create an evolution simulator that collects and continuously displays information about the objects on the map, such as the number of animals or plants. Each simulation can have a corresponding variant. During the course of the simulation the animals eat randomly generated grass, move, copulate and die when they lose all their energy. The program allows you to stop the simulation at any time on the map, highlight the animals with the most common genotype, track one of the animals by seeing the number of its children and descendants in real time, as well as the number of the simulation day on which the animal died. In addition, statistics can be exported to a CSV file.


## A more detailed description of the project (in Polish)
